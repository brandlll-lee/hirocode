...
import { Container, Text, truncateToWidth } from "@hirocode/tui";
...
function getTempFilePath(): string {
	...
	return join(tmpdir(), `hirocode-bash-${id}.log`);
}
...
/**
 * Create shell operations using hirocode's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want hirocode's
...
export function createBashToolDefinition(
	...
	return {
		name: "bash",
		label: "bash",
		description: `Execute a shell command using ${runtime.label} in the current working directory. Returns stdout and stderr. Output is truncated to last ${DEFAULT_MAX_LINES} lines or ${DEFAULT_MAX_BYTES / 1024}KB (whichever is hit first). If truncated, full output is saved to a temp file. Optionally provide a timeout in seconds.`,
		promptSnippet: `Execute ${runtime.kind === "powershell" ? "PowerShell" : runtime.label} commands`,
