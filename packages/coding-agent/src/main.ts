...
import { type ImageContent, modelsAreEqual, supportsXhigh } from "@hirocode/ai";
...
import { getRuntimeProfile, mergeAppendSystemPrompt } from "./core/benchmark-profile.js";
...
async function promptConfirm(message: string, defaultAnswer = false): Promise<boolean> {
	return new Promise((resolve) => {
...
async function createSessionManager(
	...
	runtimeProfile = getRuntimeProfile(parsed.benchmarkMode),
	...
	if (parsed.session) {
		...
		switch (resolved.type) {
			...
			case "global": {
				...
				if (runtimeProfile?.autoForkCrossProjectSession) {
					console.log(chalk.dim(`Runtime profile ${runtimeProfile.name} auto-forks cross-project sessions.`));
					return forkSessionOrExit(resolved.path, cwd, effectiveSessionDir);
				}
...
export async function main(args: string[]) {
	...
	const offlineMode =
		...
		isTruthyEnvFlag(process.env[ENV_OFFLINE_LEGACY]);
	if (offlineMode) {
		process.env[ENV_OFFLINE] = "1";
		process.env[ENV_OFFLINE_LEGACY] = "1";
		process.env[ENV_SKIP_VERSION_CHECK] = "1";
		process.env[ENV_SKIP_VERSION_CHECK_LEGACY] = "1";
	}
	...
	const runtimeProfile = getRuntimeProfile(firstPass.benchmarkMode);
	...
	const resourceLoader = new DefaultResourceLoader({
		...
		appendSystemPrompt: mergeAppendSystemPrompt(firstPass.appendSystemPrompt, runtimeProfile),
	});
	...
	let sessionManager = await createSessionManager(parsed, cwd, extensionsResult, runtimeProfile);
