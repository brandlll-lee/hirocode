...
async function withFetch(mockFetch: typeof fetch, fn: () => Promise<void>) {
...
function getShellCommand(variants: { bash: string; powershell: string; cmd: string }): string {
	const runtime = shellModule.getShellRuntime();
	if (runtime.kind === "powershell") {
		return variants.powershell;
	}
	if (runtime.kind === "cmd") {
		return variants.cmd;
	}
	return variants.bash;
}
...
describe("Coding Agent Tools", () => {
	...
	describe("bash tool", () => {
		...
		it("should prepend command prefix when configured", async () => {
			const commandPrefix = getShellCommand({
				bash: "export TEST_VAR=hello",
				powershell: "$env:TEST_VAR='hello'",
				cmd: "set TEST_VAR=hello",
			});
			const readCommand = getShellCommand({
				bash: "echo $TEST_VAR",
				powershell: "Write-Output $env:TEST_VAR",
				cmd: "echo %TEST_VAR%",
			});
			...
			const result = await bashWithPrefix.execute("test-prefix-1", { command: readCommand });
			expect(normalizeNewlines(getTextOutput(result)).trim()).toBe("hello");
		});

		it("should include output from both prefix and command", async () => {
			const commandPrefix = getShellCommand({
				bash: "echo prefix-output",
				powershell: "Write-Output prefix-output",
				cmd: "echo prefix-output",
			});
			const command = getShellCommand({
				bash: "echo command-output",
				powershell: "Write-Output command-output",
				cmd: "echo command-output",
			});
			const bashWithPrefix = createBashTool(testDir, {
				commandPrefix,
			});

			const result = await bashWithPrefix.execute("test-prefix-2", { command });
		...
		it("should expose local bash operations for extension reuse", async () => {
			...
			const chunks: Buffer[] = [];
			const command = getShellCommand({
				bash: "echo $TEST_LOCAL_BASH_OPS",
				powershell: "Write-Output $env:TEST_LOCAL_BASH_OPS",
				cmd: "echo %TEST_LOCAL_BASH_OPS%",
			});
		...
		it("should preserve executeBash sanitization when using local bash operations", async () => {
			const command = getShellCommand({
				bash: "printf '\\033[31mred\\033[0m\\r\\n'",
				powershell: 'Write-Output "$([char]27)[31mred$([char]27)[0m"',
				cmd: "echo red",
			});
			const result = await executeBash(command);

			expect(result.exitCode).toBe(0);
			expect(normalizeNewlines(result.output)).toBe("red\n");
		});
	});

	describe("webfetch tool", () => {
		it("should fetch plain text responses", async () => {
			await withFetch(
				(async () =>
				...
				async () => {
					const result = await webFetchTool.execute("test-webfetch-1", {
						url: "https://example.com/file.txt",
						format: "text",
					});

					expect(getTextOutput(result)).toBe("hello from webfetch");
		...
		it("should convert HTML to markdown", async () => {
			await withFetch(
				...
				async () => {
					const result = await webFetchTool.execute("test-webfetch-2", {
						url: "https://example.com/page",
						format: "markdown",
					});

					expect(getTextOutput(result)).toContain("Hello");
					expect(getTextOutput(result)).toContain("world");
					expect(result.details?.mimeType).toBe("text/html");
				},
			);
		});

		it("should return images as image content", async () => {
			const bytes = new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10]);
			await withFetch(
				(async () =>
					new Response(bytes, { status: 200, headers: { "content-type": "image/png" } })) as typeof fetch,
				async () => {
					const result = await webFetchTool.execute("test-webfetch-3", {
						url: "https://example.com/image.png",
					});

					expect(getTextOutput(result)).toContain("Image fetched successfully");
	...
	describe("websearch tool", () => {
		it("should parse Exa SSE search responses", async () => {
			await withFetch(
				(async () =>
					new Response(
						'data: {"jsonrpc":"2.0","result":{"content":[{"type":"text","text":"Search result body"}]}}\n\n',
						{
							status: 200,
				...
				async () => {
					const result = await webSearchTool.execute("test-websearch-1", {
						query: "latest codex cli version 2026",
					});

					expect(getTextOutput(result)).toContain("Search result body");
					expect(result.details?.provider).toBe("exa");
					expect(result.details?.numResults).toBe(8);
				},
			);
		});
	});
