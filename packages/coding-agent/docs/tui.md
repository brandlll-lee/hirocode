...
**Source:** [`@hirocode/tui`](https://github.com/badlogic/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@hirocode/tui";
...
```typescript
import { Container, type Focusable, Input } from "@hirocode/tui";
...
Import from `@hirocode/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@hirocode/tui";
```
...
```typescript
import { matchesKey, Key } from "@hirocode/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@hirocode/tui";
...
```typescript
import { getMarkdownTheme } from "@hirocode/coding-agent";
import { Markdown } from "@hirocode/tui";
...
For letting users pick from a list of options. Use `SelectList` from `@hirocode/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@hirocode/coding-agent";
import { DynamicBorder } from "@hirocode/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@hirocode/tui";
...
```typescript
import { BorderedLoader } from "@hirocode/coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@hirocode/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@hirocode/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@hirocode/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@hirocode/coding-agent";
import { matchesKey, truncateToWidth } from "@hirocode/tui";
