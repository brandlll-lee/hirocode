...
```bash
npm install @hirocode/ai
```

TypeBox exports are re-exported from `@hirocode/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@hirocode/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@hirocode/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@hirocode/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@hirocode/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@hirocode/ai';
...
```typescript
import { getModel, stream } from '@hirocode/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@hirocode/ai';
...
```typescript
import { Model, stream } from '@hirocode/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@hirocode/ai';
...
```typescript
import { getModel, complete, Context } from '@hirocode/ai';
...
```typescript
import { Context, getModel, complete } from '@hirocode/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@hirocode/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@hirocode/ai';
...
```bash
npx @hirocode/ai login              # interactive provider selection
npx @hirocode/ai login anthropic    # login to specific provider
npx @hirocode/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@hirocode/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@hirocode/ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@hirocode/ai/oauth';
...
```typescript
import { getModel, complete } from '@hirocode/ai';
import { getOAuthApiKey } from '@hirocode/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@hirocode/ai`
