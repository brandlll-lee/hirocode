...
```typescript
import { type AgentSessionEvent } from "@hirocode/coding-agent";
...
import type { MomCustomTool, ToolAPI } from "@hirocode/mom";
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `@hirocode/ai` and `@hirocode/mom` are aliased to the running mom bundle.
