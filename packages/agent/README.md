...
Stateful agent with tool execution and event streaming. Built on `@hirocode/ai`.

## Installation

```bash
npm install @hirocode/agent-core
```
...
```typescript
import { Agent, streamProxy } from "@hirocode/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@hirocode/agent-core";
